...
//create database if this first time runing app
  void createInitialData() {
    todoList = [
      ...
      {'id': const Uuid().v4(), 'title': "It`s overdue task", 'done': false, 'date': DateTime(2024, 5, 20), 'notificationId': null},
    ];
  }
...
//update the database
  ...
  void addTodoItem(String title, bool done, DateTime date, DateTime? notification, int? notificationId) {
    ...
    if (notificationId != null) {
      AwesomeNotifications().createNotification(content: NotificationContent(wakeUpScreen: true, id: notificationId, channelKey: "basic_channel", title: "ToDoDude", body: title, payload: {'route': '/calendar'}), schedule: NotificationCalendar(year: notification!.year, month: notification.month, day: notification.day, hour: notification.hour, minute: notification.minute, allowWhileIdle: true, preciseAlarm: true));
    }
    updateDataBase();
    notifyListeners();
  }
